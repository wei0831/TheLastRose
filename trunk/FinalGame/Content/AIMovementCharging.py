...
class AIMovementCharging:
    ...
    def Slamming(self):
        ...
        if self.Owner.Sprite.CurrentFrame == 3:
            ...
            self.Owner.SoundEmitter.PlayCue("SlamCue")
            ...
            if dx <= 4:
                self.Space.FindObjectByName("Camera").CameraFunction.SetCameraShake(True,.05* (4-dx),True,.05* (4-dx), .18, 0)
    ...
    def OnCollision(self, CollisionEvent):
        if CollisionEvent.OtherObject.IsSentry:
            if abs(CollisionEvent.FirstPoint.WorldNormalTowardsOther.x) > 0.95:
                #if self.reflect_cadidate < 1:
                    #self.reflect_cadidate += 1
                #else:
                self.CurrentUpdate = self.Walking
                self.dumbcounter = 30
