...
class CameraFunction:
    ...
    def FadeUpdate(self, UpdateEvent):
        ...
        #self.FadeEndFlag = self.clampCheck(0.0, result.x, self.FadeColorEnd.x)
        #if self.FadeEndFlag: self.clamp(0.0, result.x, self.FadeColorEnd.x)   
        #self.FadeEndFlag = self.clampCheck(0.0, result.y, self.FadeColorEnd.y)
        #if self.FadeEndFlag: self.clamp(0.0, result.y, self.FadeColorEnd.y)
        #self.FadeEndFlag = self.clampCheck(0.0, result.z, self.FadeColorEnd.z)
        #if self.FadeEndFlag: self.clamp(0.0, result.z, self.FadeColorEnd.z)
        #self.FadeEndFlag = self.clampCheck(0.0, result.a, self.FadeColorEnd.a)
        #if self.FadeEndFlag: self.clamp(0.0, result.a, self.FadeColorEnd.a)
        self.fade_counter -= self.deltarate
        
        if self.fade_counter <= 0:
            self.FadeLayer.Sprite.Color = self.FadeColorEnd
            #print(self.FadeColorEnd)
