...
class MouseLocationIndicator:
    ...
    AbsorbActive = Property.Bool(False)
    ...
    def Initialize(self, initializer):
        ...
        self.scale_table = dict(TreeSkillMushroom=1,TreeSkillWhirlingnut=2, TreeSkillRootplant=2, TreeSkillWorldtree=2)
    ...
    def OnMouseDown(self, MouseDownEvent):
        self.AbsorbActive = True
        
    def OnMouseUp(self, MouseUpEvent):
        self.AbsorbActive = False
    ...
    def OnRightMouseDown(self, MouseDownEvent):
        if not self.Deactivated:
            if self.TouchPlant:
                    ...
                    def DeactivateHelper(plant):
                        if plant.CanBurn:
                            plant.CanBurn.Active = False
                        if plant.CanFreeze:
                            plant.CanFreeze.Active = False
                        if plant.CanBounce:
                            plant.CanBounce.Active = False
                        if plant.CanTeleport:
                            plant.CanTeleport.Active = False
                    ...
                    DeactivateHelper(self.touched_plant)
                    if self.touched_plant.Hierarchy:
                        for child in self.touched_plant.Hierarchy.Children:
                            DeactivateHelper(child)
    ...
    def UpdateEnvironmentInfo(self):
        ...
        for contactholder in self.Owner.Collider.Contacts:
            ...
            if not self.TouchAttackable and contactholder.OtherObject.ClickReceiver and contactholder.OtherObject.ClickReceiver.Receivable and not contactholder.OtherObject.ProtectiveAnimal:
            ...
            if contactholder.OtherObject.PlantAnnihilator:
                ...
                annihilator_normals.append(contactholder.FirstPoint.WorldNormalTowardsOther)
        ...
        pi2 = math.pi/2.
        right_angled =  any(tuple(abs(az - idx*pi2) < 0.02 for idx in range(-2,3)))
        
        #print(self.touch_normal)
        if not right_angled and annihilator_points:
            self.TouchAnnihilator = True
        else:
            for an, ap in zip(annihilator_normals, annihilator_points):
                diff = (ap - self.touch_point) * Vec3(1,1,0)
                if diff.length() < 0.05 or an.x * self.touch_normal.x > .01 or an.y * self.touch_normal.y > .01:
