...
class TypeScript:
    ...
    def Initialize(self, initializer):
        ...
        if self.DestroyWhenTargetDestroyed:
            self.RegisterIt(self.DestroyWhenTargetDestroyed, lambda:self.Owner.Destroy())
        self.touched = False
        ...
        if self.Active:
            ...
            if self.emphasizer:
                ...
                if lastwordidx <= self.EmphasizeEnd+2 and lastwordidx > self.EmphasizeStart:
                    ...
                    self.emphasizer.Transform.Translation = Vec3(pos.x, pos.y - self.TestFontSize().y + 0.01, 1)
    ...
    def DestroyUpdate(self, UpdateEvent):
        self.Owner.SpriteText.Color *= VectorMath.Vec4(1,1,1,self.DestroySpeed)
        self.hider.Sprite.Color *= VectorMath.Vec4(1,1,1,self.DestroySpeed)
        if self.emphasizer:
            self.emphasizer.Sprite.Color *= VectorMath.Vec4(1,1,1,self.DestroySpeed)
