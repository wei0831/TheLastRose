...
class CameraFunction:
    ...
    # Follow Variables
    FollowEnable = Property.Bool(True)
    FollowTarget = Property.Cog()
    Follow_OffsetY = Property.Float(0)
    Follow_OffsetX = Property.Float(0)
    
    # Chase Variables
    ChaseEnable = Property.Bool(False)
    ChaseTarget = Property.Cog()
    ChaseSpeed = Property.Float(0)
    ...
    def FollowUpdate(self, UpdateEvent):
        if not self.FollowTarget == None:
            self.Owner.Transform.Translation = Vec3(self.FollowTarget.Transform.Translation.x + self.Follow_OffsetX, self.FollowTarget.Transform.Translation.y + self.Follow_OffsetY, self.Owner.Transform.Translation.z)
            if self.ShakeEnable:
